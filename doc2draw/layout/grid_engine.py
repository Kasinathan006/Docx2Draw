...
# Adaptive widths/gaps so layouts with more columns still fit on-screen
_COL_WIDTH_MAP = {1: 900, 2: 720, 3: 600, 4: 480, 5: 400, 6: 340}
_COL_GAP_MAP  = {1:   0, 2:  60, 3:  60, 4:  50, 5:  40, 6:  30}

class GridEngine:
    def __init__(
        self,
        col_width: float = 600.0,
        col_gap: float = 60.0,
        row_gap: float = 60.0,
        start_x: float = 100.0,
        start_y: float = 80.0,
        ...
        # Always override with the adaptive preset so callers only need max_cols
        self.col_width = float(_COL_WIDTH_MAP.get(max_cols, col_width))
        self.col_gap   = float(_COL_GAP_MAP.get(max_cols, col_gap))
    ...
    def calculate_card_height(self, item: DiagramItem) -> float:
        """Dynamically calculates card height based on content and image presence."""
        chars_per_line = max(30, int(self.col_width / 7.5))
        base_height = 100.0  # title + separator
        line_height = 26.0
        for pt in item.bullet_points:
            lines = max(1, (len(pt) + chars_per_line - 1) // chars_per_line)
            base_height += lines * line_height
        if item.screenshot_ref:
            img_h = min(int(self.col_width * 9 / 16), 320)
            base_height += img_h + 40  # 40px padding around image
    ...
    def generate_layout(self, structure: DiagramStructure) -> Tuple[List[dict], Dict[str, str]]:
        ...
        """
        # col_width is already set correctly in __init__; just alias
        current_col_width = self.col_width
        
        elements = []
        files = {}
        
        # 1. Header Banner
        banner_width = self.max_cols * current_col_width + (self.max_cols - 1) * self.col_gap
        banner_height = 160.0
        ...
        for idx, item in enumerate(structure.items):
            ...
            x = self.start_x + col_idx * (current_col_width + self.col_gap)
            ...
            card_rect = create_element(item.id, "rectangle", x, y, current_col_width, height, palette["bg"], palette["stroke"])
            ...
            if item.screenshot_ref:
                ...
                if data_url:
                    ...
                    img_margin = 20.0
                    img_w = current_col_width - img_margin * 2
                    img_h = float(min(int(current_col_width * 9 / 16), 320))
                    img_x = x + img_margin
                    # Place image near bottom of card with 20px bottom padding
                    img_y = y + height - img_h - 20.0
            ...
            elements.append(create_bound_text(item.id, card_text, x, y, current_col_width, height, 16, palette["text"]))
            
            # Save position for arrow routing
            card_positions[item.id] = (x, y, current_col_width, height)
